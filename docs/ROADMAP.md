...
- [x] **CSRF token validation** — Synchronizer Token Pattern. Token generated on login/signup, stored in iron-session. Middleware validates `X-CSRF-Token` on all POST/PATCH/DELETE. Auth routes exempt. Client-side: `apiFetch()` wrapper in `lib/api-fetch.ts` automatically fetches, caches, and sends the token on all mutating requests.
